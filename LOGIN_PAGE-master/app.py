...
engine = create_engine("postgres://postgres:prasanna@localhost/Goodreads")
db = scoped_session(sessionmaker(bind=engine))
key = os.getenv("aiuUklDP3oBTbgHwobdZeg")
...
def register():
    ...
    # if POST, validate and commit to database
        ...
        ## end validation
            ...
            try:
                db.execute("INSERT INTO user_registration (firstname, lastname, email, password) VALUES (:firstname, :lastname, :email, :password)",
                               {"firstname": first_name, "lastname": last_name, "email":email, "password": generate_password_hash(password, method='pbkdf2:sha512')})
...
def login():
    ...
    if request.method == "POST":
        form_name = request.form.get("first_name")
        ...
        Q = db.execute("SELECT * FROM user_registration WHERE firstname LIKE :first_name", {"first_name": form_name}).fetchone()
...
def mybooks():
    if request.method == "GET":
        query = session["firstname"]
        result = db.execute("SELECT * FROM mybooks WHERE username = :username", {"username": query}).fetchall()  
        return render_template("mybooks.html", result=result)

@app.route("/addbooks/<int:bookid>", methods=["GET","POST"])
@login_required
def addbooks(bookid):
    if request.method == "GET":
        query = db.execute("SELECT * from books WHERE bookid = :bookid", {"bookid": bookid}).fetchone()
        Yes = "Yes"
        No = "No"
        try:
            result = db.execute("INSERT INTO mybooks (listofbooks, username, booksnotread, booksread) VALUES (:listofbooks, :username, :booksnotread, :booksread)", {"listofbooks": query.title, "username": session["firstname"], "booksnotread": Yes, "booksread": No})
        except Exception as e:
            return render_template("error.html", message = e)
        
        return render_template("success.html", message="Your book has been added")
...
def details(bookid):
    if request.method == "GET":
        ...
        try:
            goodreads = requests.get("https://www.goodreads.com/book/review_counts.json", params={"key": "aiuUklDP3oBTbgHwobdZeg", "isbns": result.isbn})
        except Exception as e:
            return render_template("error.html", message = e)

        # Get comments particular to one book
        comment_list = db.execute("SELECT u.firstname, u.lastname, u.email, r.rating, r.comment from reviews r JOIN user_registration u ON u.userid=r.user_id WHERE book_id = :id", {"id": bookid}).fetchall()
        if not result:
...
def api(isbn):
    ...
    goodreads = requests.get("https://www.goodreads.com/book/review_counts.json", params={"key": "aiuUklDP3oBTbgHwobdZeg" , "isbns": isbn})
